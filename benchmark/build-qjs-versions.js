...
const OUTPUT_DIR = path.join(SCRIPT_DIR, 'quickjs-bin', PLATFORM_DIR)
...
function ensureQuickjsRepo() {
  if (!fs.existsSync(QJS_DIR)) {
    ...
    if (!fs.existsSync(gitDir)) {
      console.error(
        `Existing quickjs directory at ${QJS_DIR} is not a git repository.`
      )
      process.exit(1)
    }

    console.log(
      'Updating existing QuickJS repository (checkout master/main then pull)...'
    )
    try {
      ...
      if (!checkoutSucceeded) {
        console.error(
          'Failed to checkout master or main in QuickJS repository.'
        )
        process.exit(1)
      }
      ...
      console.error(
        'Failed to update QuickJS repository. Please resolve issues and try again.'
      )
      console.error(err.message)
      process.exit(1)
    }
...
function ensureCleanWorkingTree() {
  try {
    ...
    console.error(
      `Working tree is not clean in ${QJS_DIR}. Please commit or stash changes first.`
    )
    process.exit(1)
  }
}
...
function main() {
  ...
  try {
    const logOutput = runGit(
      'git log --format="%H %ad" --date=short --since="2025-04-26"'
    )
      ...
      if (fs.existsSync(OUTPUT_DIR)) {
        ...
        if (files.length === 0) {
          ...
          for (const name of files) {
            ...
            console.log(
              `${stat.mode.toString(8)} ${stat.size
                .toString()
                .padStart(8, ' ')} ${fullPath}`
            )
          }
        ...
        if (platforms.length > 1) {
          ...
          for (const platform of platforms) {
            ...
            const count = fs
              ...
              .filter((n) => n.startsWith('qjs-')).length
